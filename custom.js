...
$( document ).ready(function() {
	...
	try {
		...
		var callback = function(mutationsList) {
			...
			if($('.add-to-cart.c-button.c-product-details__add-to-cart-button.add-to-cart').length  > 2) {
			...
			if((skatesproductpage || helmetsproductpage) && $('.disclaimer-popup').length  == 1) {
			...
			}else if((skatesproductpage || helmetsproductpage) && clicked ){
				disclaimerPopUP();
				$('#add-to-cart.c-button.c-product-details__add-to-cart-button.add-to-cart').on('click', disableClick);
			}
		}
